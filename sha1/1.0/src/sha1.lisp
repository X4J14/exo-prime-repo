...
(defparameter *read-stream-buffer-size* 1024)
...
(defun sha1-digest-stream (stream)
  "Return the SHA1 digest for a stream."
  (digest-stream stream))

;;; ----------------------------------------------------

(defun sha1-digest-file (file-path)
...
(defun sha1-hex-stream (stream)
  "Return the SHA1 hex digest for a stream."
  (format nil "~{~16,2,'0r~}" (sha1-digest-stream stream)))
  
;;; ----------------------------------------------------

(defun sha1-hex-file (file-path)
  "Return the SHA1 hex digest for a file."
  (format nil "~{~16,2,'0r~}" (sha1-digest-file file-path)))
