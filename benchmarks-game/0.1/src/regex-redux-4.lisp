...
;; https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/regexredux-sbcl-4.html
;;
...
#+sb-thread
(progn
  (a:define-alien-routine sysconf a:long (name a:int))
  (use-package :sb-thread)))

(eval-when (:compile-toplevel)
  (setf ppcre:*regex-char-code-limit* 128))
  ;(setf *regex-char-code-limit* 128))
...
#+sb-thread
(defun parts (parts-num len
     &aux (ranges (loop with (step rest) of-type fixnum =  (multiple-value-list (floor len parts-num))
...
(defun replace-all (regexp replacement target-string
     &aux (rmatch '())
          (match '())
...
(defun main (&optional (pifile #p"/dev/stdin"))
  (let* ((stream (open pifile :external-format :iso-8859-1))
         (sequence (read-all stream))
  ...
  (progn
    (loop for regex in +regex-list+ do
         (format t "~a ~a~%" regex
                 (/ (length
                     (the list
                       (ppcre:all-matches regex sequence))) 2)))
          ...
          (bg (let (reg cur)
              ...
              (setf (nth cur result)
                    (format nil "~a ~a" reg
                            (/ (length
                                (the list
                                  (ppcre:all-matches reg sequence))) 2))))))
