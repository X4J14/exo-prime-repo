...
;; https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/revcomp-sbcl-1.html
;;
...
(defun main (&optional (pifile #p"/dev/stdin"))
  (declare (optimize (speed 3) (safety 0)))
  (with-open-file (in pifile :element-type +ub+)
    (let ((i-buf (make-array +buffer-size+ :element-type +ub+))
      ...
      (flet ((flush-chunks ()
            ...
            do (setf (aref o-buf k) (aref +lut+ byte) j (1+ j) k (1+ k))
                                ...
                                if (> k (- +buffer-size+ 2)) do (write-sequence o-buf *standard-output* :end (shiftf k 0)))
                       finally (when (plusp k)
                                 (write-sequence o-buf *standard-output* :end k))
                               (when (plusp j)
                                 (write-byte +newline+ *standard-output*)))
                 (setf chunks nil))))
        (prog ((start 0) (end 0))
         read-chunk
           ...
           (let ((bod (position #.(char-code #\>) i-buf :start start :end end)))
             (cond ((numberp bod)
                    ...
                    (loop for eod = (position +newline+ i-buf :start start :end end)
                          do (cond ((numberp eod)
                                    (write-sequence i-buf *standard-output* :start start :end (incf eod))
                                    (setf start eod)
                                    (go parse-chunk))
                                   (t (write-sequence i-buf *standard-output* :start start :end end)
                                      (setf start 0)
                                      (setf end (read-sequence i-buf in))))))
