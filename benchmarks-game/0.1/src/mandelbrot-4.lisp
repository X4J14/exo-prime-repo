...
; keep in mind current exo package
(defparameter exo-tmp::pkg *package*)
...
;; https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/mandelbrot-sbcl-4.html
;;
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  #+x86-64
    (handler-bind ((simple-error (lambda (condition)
      ...
      (sb-c::defknown complex-double-float/sse-* ((complex double-float)
                     (complex double-float))
        (complex double-float))
      (sb-c::defknown cmple-movmskpd ((complex double-float)
                   (complex double-float))
        (unsigned-byte 32))
...
#+x86-64
(declaim (inline #.(intern (string-upcase '#:complex-double-float/sse-*) exo-tmp::pkg)))
#+x86-64
(defun #.(intern (string-upcase '#:complex-double-float/sse-*) exo-tmp::pkg) (numbera numberb)
  (declare (type (complex double-float) numbera numberb)
           (optimize (speed 3) (safety 0) (space 0) (debug 0)))
  (complex-double-float/sse-* numbera numberb))

#+x86-64
(declaim (inline #.(intern (string-upcase '#:cmple-movmskpd) exo-tmp::pkg))))
#+x86-64
(defun #.(intern (string-upcase '#:cmple-movmskpd) exo-tmp::pkg) (numbera numberb)
  (declare (type (complex double-float) numbera numberb)
           (optimize (speed 3) (safety 0) (space 0) (debug 0)))
  (cmple-movmskpd numbera numberb))

; back to current exo package
(in-package #.(package-name exo-tmp::pkg))
...
(defmacro escapes? (n two-pixels  crv civ)
  (let ((escaped (gensym "escaped"))
    ...
    `(let ((,zrv +zero+)
      ...
      (,tiv +zero+))
       (block ,escaped
    ,@(nreverse (loop for i of-type fixnum from 0 below n
         collecting
         `(progn
...
(declaim (inline calc-row))
(defun calc-row (y n bitmap bytes-per-row crvs inverse-h)
               ...
               (lambda ()
                 (loop for y of-type fixnum
                  ...
                  do (calc-row y n
                     ...
                     crvs inverse-h)))))))
    (write-sequence (cbyte (format nil "P4~%~d ~d~%" n n)) *standard-output*)
    (write-sequence bitmap *standard-output*));)
  (values)))