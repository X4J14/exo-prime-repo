...
;; https://benchmarksgame-team.pages.debian.net/benchmarksgame/program/binarytrees-sbcl-1.html
;;
  ...
  (loop for d of-type fixnum from min-depth by 2 upto max-depth do
       (loop with iterations of-type fixnum = (ash 1 (+ max-depth min-depth (- d)))
          ...
          into result of-type fixnum
            ...
            (format *standard-output* "~D	 trees of depth ~D	 check: ~D~%"
...
(defun main (&optional (n (parse-integer
  ...
  (format *standard-output* "stretch tree of depth ~D	 check: ~D~%" (1+ n) (check-node (build-btree 0 (1+ n))))
  (let ((*print-pretty* nil) (long-lived-tree (build-btree 0 n)))
    ;(purify)
    (loop-depths n)
    (format *standard-output* "long lived tree of depth ~D	 check: ~D~%" n (check-node long-lived-tree))))
