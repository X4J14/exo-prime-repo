(exo (:export :intern #:exo-run #:main))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  #+x86-64(handler-bind ((simple-error (lambda (condition)
      ...
      (unsigned-byte 32))
       (sb-vm::define-vop (complex-double-float/sse-*)
         (:policy :fast-safe)
...
#+x86-64(defmacro escapes? (n two-pixels  crv civ)
  (let ((escaped (gensym "escaped"))
        ...
        (trv (gensym))
        (tiv (gensym)))
     `(let ((,zrv +zero+) (,ziv +zero+) (,trv +zero+) (,tiv +zero+))
        (block ,escaped
     ,@(nreverse (loop for i from 0 below n
          ...
          `(progn
             ...
             (let ((,temp2 (+ ,trv ,tiv)))
               (setf ,two-pixels (cmple-movmskpd ,temp2 ,+four+)))
             (when (= ,two-pixels 0)
               (return-from ,escaped)))
          ))))))
