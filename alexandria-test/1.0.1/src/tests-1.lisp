...
(defun run-tests (&key ((:compiled sb-rt::*compile-tests*)))
  (do-tests))
...
(eval-when (:compile-toplevel :load-toplevel :execute)
  (pushnew 'curry.3 sb-rt::*expected-failures*))
...
(deftest binomial-coefficient.2
    (binomial-coefficient 2000000000000 20)
    ...
    (let ((data "sdkfjhsakfh weior763495ewofhsdfk sdfadlkfjhsadf woif sdlkjfhslkdfh sdklfjh"))
      (values (equal data
              ...
              (equal (subseq data 10 20)
                     (with-input-from-string (in data)
                       (with-output-to-string (out)
                         (copy-stream in out :start 10 :end 20))))
              (equal (subseq data 10)
                     (with-input-from-string (in data)
                       (with-output-to-string (out)
                         (copy-stream in out :start 10))))
              (equal (subseq data 0 20)
                     (with-input-from-string (in data)
                       (with-output-to-string (out)
                         (copy-stream in out :end 20))))))
  t
  t
  t
  t)
